    ...
    if (isboolture) {
            ...
            $.post("../timeLimit/createTimeLimitTable", ajaxPram , function (response) {
                ...
                if (odj['backFlag'] == "ok") {
                    ...
                    $.ajax({
                        type: "POST",
                        url:  "../timeLimit/createTimeLimitPdf",
                        data: {
                            RecordId: record_id,
                            ID:ID
                        },
                        ...
                        success: function (data) {
                            if (data.msg == "ok") {
                               location.href="../page/SuccessfulTimeLimit?RecordId="+record_id+"&social_credit_code="+social_credit_code+"&userId="+userId+"&sCode="+sCode+"&ID="+ID;
                            }
                            else {
                                alert("提交失败，请重新提交");
                    ...
                    // location.href="../page/lawEnforcementDocument?RecordId="+record_id+"&social_credit_code="+social_credit_code+"&userId="+userId+"&ID="+ID+"&sCode="+sCode;
                } else {
                    alert(odj['failInfo']);
                }
            },"json");
