...
public interface FireIndexDao {
    ...
    @Select("select CHECK_ITEM from FIRE_CHECK_ITEM where CREATE_TIME=#{CREATE_TIME}")
    String getCheckItem(@Param("CREATE_TIME") String CREATE_TIME);
    @Select("select CHECK_UNIT from FIRE_CHECK_ITEM where CREATE_TIME=#{CREATE_TIME}")
    String getCheckUnit(@Param("CREATE_TIME") String CREATE_TIME);
    @Select("SELECT rank from (SELECT @rank := @rank + 1 as rank, CREATE_TIME from (SELECT CREATE_TIME FROM FIRE_CHECK_ITEM where CHECK_UNIT='栖霞区人民政府马群街道办事处' ORDER BY CREATE_TIME) as a,(SELECT @rank := 0) b) as c where CREATE_TIME=#{CREATE_TIME} ")
    String getCheckUnitTable(@Param("CHECK_UNIT") String CHECK_UNIT, @Param("CREATE_TIME") String CREATE_TIME);
}
